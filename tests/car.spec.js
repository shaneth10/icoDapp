...
describe('contract', () => {
  ...
  it('has initial brand', async () => {
      const brand = await contract.methods.brand().call();//调用合约上的方法，通常是取数据，立即返回
      assert.equal(brand, initialBrand);
  });

  it('can change the brand', async () => {
      const newBrand = 'BWM';
      await contract.methods.setBrand(newBrand).send({ from: accounts[0] });//对合约发起交易，通常是修改数据，返回的是交易 Hash
